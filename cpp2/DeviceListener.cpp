...
HRESULT DeviceListener::RegisterForEvents(HWND hwnd)
{
	...
	RETURN_IF_FAILED(_configurationInterop->GetForWindow(hwnd, IID_PPV_ARGS(&_configuration)));
...
HRESULT DeviceListener::SetDefaultMenyItem()
{
	auto empty = Microsoft::WRL::Make<Supplimental::WRL::Collections::Vector<ABI::Windows::UI::Input::RadialControllerSystemMenuItemKind>>();
	_configuration->SetDefaultMenuItems(empty.Get());
...
HRESULT DeviceListener::RemoveMenuItem()
{
	// Get menu items
	ComPtr<Collections::IVector<RadialControllerMenuItem*>> menuItems;
	RETURN_IF_FAILED(_menu->get_Items(&menuItems));
	...
	swprintf_s(message, 2000, L"Clear menu\n");
